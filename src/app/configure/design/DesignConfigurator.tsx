...
import { saveConfig as _saveConfig, SaveConfiArgs } from "./actions";
...
const DesignConfigurator = ({
  ...
  const {mutate: saveConfig} = useMutation({
    ...
    mutationFn: async (args: SaveConfiArgs) => {
      await Promise.all([saveConfiguration(), _saveConfig(args)])
    },
    onError: () => {
      toast({
        title: "Algo deu errado",
        description: "Temos um erro, tente novamente mais tarde.",
        variant: "destructive"
      })
    },
    onSuccess: () => {
                    ...
                    <DropdownMenuContent>
                      {MODELS.options.map((model) => (
                        <DropdownMenuItem
                          key={model.label}
                          className={cn(
                            "cursor-pointer flex text-sm gap-1 items-center p-1.5 hover:bg-zinc-100",
                            {
                              "bg-zinc-100":
